...
if [ $stage -le 3 ]; then
  echo "$0: Performing RNNLM rescoring decoding results"
  for lm in tgsmall tgmed tglarge fglarge; do
    for devset in test_clean test_other dev_clean dev_other; do
      
      for system in nnet5a_clean_100_gpu; do
        sourcedir=exp/${system}/decode_${lm}_${devset}
        if [ ! -d "$sourcedir" ]; then
...
#	rm -rf {resultsdir}
#	steps/lmrescore_rnnlm_lat.sh --skip_scoring false --rnnlm_ver $rnnlm_ver data/lang_test_$lm $modeldir data/$devset $sourcedir ${resultsdir}
