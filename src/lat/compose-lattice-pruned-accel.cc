...
#include "lat/compose-lattice-pruned-accel.h"
...
/**
   PrunedCompactLatticeAccelComposer implements an algorithm for pruned composition.
 ...
 */
class PrunedCompactLatticeAccelComposer {
 public:
  PrunedCompactLatticeAccelComposer(
      const ComposeLatticePrunedAccelOptions &opts,
  ...
  void Compose();
  void getStatistics(ComposeLatticePrunedAccelStats &stats);
 ...
 private:
  ...
  struct LatticeStateInfo {
    ...
    // 'arcs_to_end' is the minimum number of arcs from the state to any
    // final state
    int arcs_to_end;
  ...
  const ComposeLatticePrunedAccelOptions &opts_;
  ...
  ComposeLatticePrunedAccelStats stats_;
};


void PrunedCompactLatticeAccelComposer::GetTopsortedStateList(
...
int32 PrunedCompactLatticeAccelComposer::GetCurrentArcLimit() const {
...
void PrunedCompactLatticeAccelComposer::RecomputePruningInfo() {
...
void PrunedCompactLatticeAccelComposer::ComputeForwardCosts(
...
void PrunedCompactLatticeAccelComposer::ComputeBackwardCosts(
    const std::vector<int32> &composed_states) {
  ...
  for (; iter != end; ++iter) {
    ...
    // Get the associated lattice state for the src_state
    int32 src_lat_state = composed_state_info_[composed_state_index].lat_state;
    LatticeStateInfo &src_lat_info = lat_state_info_[src_lat_state];
    ...
    for (; !aiter.Done(); aiter.Next()) {
      ...
      // Get the associated lattice state for the dst_state
      int32 dst_lat_state = composed_state_info_[arc.nextstate].lat_state;
      LatticeStateInfo &dst_lat_info = lat_state_info_[dst_lat_state];
      
      // src_state.num_arcs = dst_state.num_arcs + 1
      src_lat_info.arcs_to_end = dst_lat_info.arcs_to_end + 1;
    }
  ...
  //int32 lat_state = composed_state_info_[0].lat_state;
  //output_best_cost_ = composed_state_info_[0].backward_cost + lat_state_info_[lat_state].arcs_to_end*opts_.arcs_to_end_scale;
...
void PrunedCompactLatticeAccelComposer::ComputeDeltaBackwardCosts(
    const std::vector<int32> &composed_states) {
  ...
  for (; iter != end; ++iter) {
    ...
    //BaseFloat expected_cost_offset = lat_state_info_[info.lat_state].arcs_to_end*opts_.arcs_to_end_scale;
    ...
    if (expected_cost_offset < current_cutoff) {
      expected_cost_offset += lat_state_info_[info.lat_state].arcs_to_end*opts_.arcs_to_end_scale;
...
void PrunedCompactLatticeAccelComposer::ComputeLatticeStateInfo() {
  ...
  for (int32 s = num_lat_states - 1; s >= 0; s--) {
    ...
    int arcs_to_end = INT_MAX;
    if (backward_cost != std::numeric_limits<double>::infinity()) {   // Actually a final state
    ...
    for (; !aiter.Done(); aiter.Next(), ++arc_index)  {
      ...
      if (lat_state_info_[arc.nextstate].arcs_to_end < arcs_to_end) {
        arcs_to_end = lat_state_info_[arc.nextstate].arcs_to_end;
      }
    }
    ...
    info.arcs_to_end = arcs_to_end + 1;  // This is the state's computing cost,
                                         // reflecting the cost from this state to any path connected
                                         // to an end state
    std::cerr << "arc_to-end: " << info.arcs_to_end << std::endl;
  ...
  // We dont include arc_to_end here, because that value should only be used to order the
  // arcs, not to prune them
  lat_best_cost_ = lat_state_info_[0].backward_cost;
}

PrunedCompactLatticeAccelComposer::PrunedCompactLatticeAccelComposer(
      const ComposeLatticePrunedAccelOptions &opts,
...
void PrunedCompactLatticeAccelComposer::AddFirstState() {
...
void PrunedCompactLatticeAccelComposer::ProcessQueueElement(
  ...
  { // this block update the state's 'sorted_arc_index', 'arc_delta_cost' and
    ...
    if (expected_cost_offset < current_cutoff_) {
      ...
      // Add arcs_to_end to heuristic so we take computation cost
      // into account when deciding which arc to expand first
      expected_cost_offset += lat_state_info.arcs_to_end*opts_.arcs_to_end_scale;
...
void PrunedCompactLatticeAccelComposer::ProcessTransition(int32 src_composed_state,
                                                     int32 arc_index) {
  ...
  { // The next block works out 'dest_composed_state' and
    ...
    if (ret.second) {
      ...
      //BaseFloat expected_cost_offset = dest_lat_state_info.arcs_to_end*opts_.arcs_to_end_scale;
      cerr << "num_arcs: (" << num_arcs_out_ << " of " << GetCurrentArcLimit() << ") ";
      cerr << "[" << src_composed_state << "," << arc_index << " ";
      cerr << "Expected cost: " << expected_cost_offset  << "(" << dest_lat_state_info.arcs_to_end*opts_.arcs_to_end_scale  << "), Curr cutoff: " << current_cutoff_;
      if (expected_cost_offset < current_cutoff_) {
        cerr << " [GOOD]" << std::endl;
        // the following call should be equivalent to
        // composed_state_queue_.push(std::pair<BaseFloat,int32>(...)) with
        // the same pair of args.
        expected_cost_offset += dest_lat_state_info.arcs_to_end*opts_.arcs_to_end_scale;
...
void PrunedCompactLatticeAccelComposer::getStatistics(ComposeLatticePrunedAccelStats &stats) {
   stats.copyFrom(stats_);
}


void PrunedCompactLatticeAccelComposer::Compose() {
  ...
  // This loop is going to process states until one of the following conditions:
  //  - reached_max_arcs AND have_at_least_one_complete_path
  //  - no_more_states_to_process  
  while (output_best_cost_ == std::numeric_limits<double>::infinity() ||
    ...
    while (num_arcs_out_ < this_iter_arc_limit &&
      ...
      cerr << "best_cost: " << output_best_cost_ << ", num_arcs_out: " << num_arcs_out_ << std::endl;
      cerr << "num_arcs_out_: " << num_arcs_out_ << ", this_iter_arc_limit: " << this_iter_arc_limit << std::endl;
    }
...
void ComposeCompactLatticeAccelPruned(
    const ComposeLatticePrunedAccelOptions &opts,
    const CompactLattice &clat,
    fst::DeterministicOnDemandFst<fst::StdArc> *det_fst,
    CompactLattice* composed_clat,
    ComposeLatticePrunedAccelStats* stats) {

  PrunedCompactLatticeAccelComposer composer(opts, clat, det_fst, composed_clat);
  composer.Compose();
