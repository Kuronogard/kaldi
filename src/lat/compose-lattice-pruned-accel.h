...
#ifndef KALDI_LAT_COMPOSE_LATTICE_PRUNED_ACCEL_H_
#define KALDI_LAT_COMPOSE_LATTICE_PRUNED_ACCEL_H_
...
struct ComposeLatticePrunedAccelOptions {
  ...
  double arcs_to_end_scale;

  double measure_period;

  ComposeLatticePrunedAccelOptions(): lattice_compose_beam(6.0),
                                 max_arcs(100000),
                                 initial_num_arcs(100),
                                 growth_ratio(1.5),
                                 arcs_to_end_scale(1.0),
  ...
  void Register(OptionsItf *po) {
    ...
    po->Register("initial-num-arcs", &initial_num_arcs, "Number of arcs to expand before the first pruning");
    ...
    po->Register("arcs-to-end-scale", &arcs_to_end_scale, "Scale applied to the number of arcs from each state to any final state");
  }
};

struct ComposeLatticePrunedAccelStats {
    ...
    ComposeLatticePrunedAccelStats(): 
    ...
    void copyFrom(ComposeLatticePrunedAccelStats &other) {
...
void ComposeCompactLatticeAccelPruned(
    ...
    ComposeLatticePrunedAccelStats* stats = NULL);
