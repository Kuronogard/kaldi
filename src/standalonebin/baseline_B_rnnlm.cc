...
int main(int argc, char *argv[]) {
  try {
   ...
   //po.Register("acoustic-scale", &acoustic_scale, "Scaling factor for acoustic likelihoods");
	po.Register("use-gpu", &use_gpu, "Use GPU or not for nnet computations (yes|no) (default:no)");
    ...
    std::string ac_model_filename = po.GetArg(1),
        decode_fst_filename = po.GetArg(2),
				...
				rnnlm_filename = po.GetArg(7),
				word_embedding_filename = po.GetArg(8),
				time_log_filename = po.GetArg(9);
    ...
    TransitionModel trans_model;
      ...
      trans_model.Read(ki.Stream(), binary);
		...
		const rnnlm::RnnlmComputeStateInfo info(rnnlm_opts, rnnlm, word_embedding_mat);
		...
		fst::DeterministicOnDemandFst<StdArc> *lm_to_add = new rnnlm::KaldiRnnlmDeterministicFst(max_ngram_order, info);
