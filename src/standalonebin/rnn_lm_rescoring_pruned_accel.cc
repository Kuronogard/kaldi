...
int main(int argc, char **argv) {
  ...
  ComposeLatticePrunedAccelOptions compose_pruned_opts;
  ...
  if (lat_wspecifier != "") {
	...
	try {
      ...
      ComposeLatticePrunedAccelStats latComposeStats;
			CompactLattice composed_clat;
			ComposeCompactLatticeAccelPruned(compose_pruned_opts, clat, &combined_lms, &composed_clat, &latComposeStats);
      ...
      if (clattice_writer.IsOpen()) {
        ...
        int32 num_states = composed_clat.NumStates();
        int32 num_arcs = 0;
        for (int32 s = 0; s < num_states; s++)
          num_arcs += composed_clat.NumArcs(s);

        std::cerr << "States: " << num_states << ", Arcs: " << num_arcs << std::endl;
